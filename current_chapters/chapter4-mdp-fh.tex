...
The probability ${p_\ttime}(\state'|\state,\action)$ is the \emph{transition probability} from state $\state$ to state $\state'$ for a given action $\action$. We naturally require that
            ${p_\ttime}(\state'|\state,\action) \ge 0$, and $\sum_{\state' \in {\States_{\ttime + 1}}}^{} {{p_\ttime}(\state'|\state,\action)}  = 1$ for all $\state \in {\States_\ttime},\action \in {\Actions_\ttime}(\state)$.
...
expectation operator is denoted by ${\E^{\policy ,{\transitionprob_0}}}( \cdot )$, where the expectation is over state-action seqeunces generated using the stochastic
...
define:
\[\Value_\tHorizon^\policy (\state) = {\E^\policy }\left(\sum_{\ttime = 0}^\tHorizon {{R_\ttime}} |{\state_0} = \state\right) \equiv {\E^{\policy ,s}}\left(\sum_{\ttime = 0}^\tHorizon {{R_\ttime}} \right) .\]
...
\[\Value_\discount ^\policy (\state) = {\E^\policy }\left(\sum_{\ttime = 0}^\infty  {{\discount ^t}\reward({\state_\ttime},{\action_\ttime})} |{\state_0} = \state\right) \equiv {\E^{\policy ,s}}\left(\sum_{\ttime = 0}^\infty  {{\discount ^t}\reward({\state_\ttime},{\action_\ttime})} \right)\;,\]
