...
Recall our definition of the $\QValue$-function (or {\em state-action
...
%\AT{Yishay can you please clarify this transition? I don't see how it results from Lemma 10.2}\YM{I updated Lemma 10.2 for the relative bound. I changes the constants  of the $n$ and $m$, need to check where we use it . I am confused why I need $2|\States$ and not only $|\States$.}
...
\begin{figure}
\begin{center}
\includegraphics[width=0.5\linewidth]{figures/2state-general.PNG}    
\end{center}
...
\]
%\AT{I don't understand the point of this section - the result is a consequence of how we define the SE. Had we counted only the first visits, we would have gotten first visit. In general, the empirical mean is the minimizer of MSE - is that what we want to say here?}
%\YM{The point is to show that Every Visit minimizes a fairly natural loss. We have Every Visit run over the entire sample, and not "reduced samples" as we need for the First Visit.}
...
For the control, we can maintain an estimates of the $\QValue^\policy$
...
We call a sequence of learning rates $\{\alpha_\ttime(\state)\}$ is \emph{well formed} if for every $\state\in\States$ we have: (1) $\sum_\ttime
\alpha_\ttime(\state)
...
%For every $(\state,\action)$ we have (1) $\sum_\ttime
...
% \YM{Maybe add a reference where the proofs of Theorems 11.6 and 11.7 appear}
...
\YM{Need to explain what we mean by convergence, given a fixed sample S. Namely resampling from S.}
...
%The proof of the convergence of $TD(0)$ is very similar to that of $Q$-learning. 
...
%\footnote{\YM{YM: needed to check if the example is from Sutton}}
...
\subsection{$\QValue$-learning: Markov Decision Process}
...
% (We will not give the details of the
...
\begin{align*}
  ...
  %\\
  %&+\epsilon[\frac{1}{|\Actions|}\sum_{b\in\Actions}[\reward(\state,b)+\discount E_{\state'\sim\transitionprob(\cdot|\state,b)}[\frac{\epsilon}{|\Actions|}\sum_{b'\in\Actions} q(\state',b')+ (1-\epsilon)\max_{\action'\in\Actions}q(\state',\action')]]  
\end{align*}
...
%    &+\discount(1-\epsilon)\epsilon \frac{1}{|\Actions|}\sum_{b\in\Actions} E_{\state'\sim\transitionprob(\cdot|\state,b)}[\max_{\action'\in\Actions}q_1(\state',\action')-\max_{\action''\in\Actions}q_2(\state',\action'')]\\
 %   & + \discount\epsilon^2  E_{\state'\sim\transitionprob(\cdot|\state,b)}[\frac{\epsilon}{|\Actions|}\sum_{b'\in\Actions} q(\state',b') \\
%        \leq & \discount(1-\epsilon)\|q_1-q_2\|_\infty +\discount\epsilon \|q_1-q_2\|_\infty   = 
    \leq & \discount\|q_1-q_2\|_\infty\;.
\end{align*}
...
\begin{align*}
 ...
 % &+\epsilon[\frac{1}{|\Actions|}\sum_{b\in\Actions}[\reward(\state,b)+\discount E_{\state'\sim\transitionprob(\cdot|\state,b)}[\frac{\epsilon}{|\Actions|}\sum_{b'\in\Actions} Q^{*,\epsilon}(\state',b')+ (1-\epsilon)\max_{\action'\in\Actions}Q^{*,\epsilon}(\state',\action')]]  
 % 
%  (1-\epsilon)[\reward(\state,\action)+\discount E_{\state'\sim\transitionprob(\cdot|\state,\action)}[\max_{\action'\in\Actions}Q^{*,\epsilon}(\state',\action')] \\
%  &+\epsilon[\frac{1}{|\Actions|}\sum_{b\in\Actions}[\reward(\state,b)+\discount E_{\state'\sim\transitionprob(\cdot|\state,b)}[\max_{\action'\in\Actions}Q^{*,\epsilon}(\state',\action')]]   
\end{align*}
...
\begin{align*}
    Q^{*,\epsilon}(\state,\action) -Q^{*}(\state,\action) =& %\epsilon(\frac{1}{|\Actions|}\sum_{b\in\Actions}\reward(\state,b)-\reward(\state,\action))\\
...
%    &+  \epsilon(1-\epsilon)\discount   ( E_{\state'\sim\transitionprob(\cdot|\state,b)}[\max_{\action'\in\Actions}Q^{*,\epsilon}(\state',\action')] - E_{\state'\sim\transitionprob(\cdot|\state,\action)}[\max_{\action'\in\Actions}Q^{*}(\state',\action')])
%    \\   &+
%    \epsilon^2\discount (E_{\state'\sim\transitionprob(\cdot|\state,b)}[\frac{\epsilon}{|\Actions|}\sum_{b'\in\Actions} Q^{*,\epsilon}(\state',b') ]-E_{\state'\sim\transitionprob(\cdot|\state,\action)}[\max_{\action'\in\Actions}Q^{*}(\state',\action')])
 %   \\
 %   
%  &+\epsilon\discount[\frac{1}{|\Actions|}\sum_{b\in\Actions} E_{\state'\sim\transitionprob(\cdot|\state,b)}[\max_{\action'\in\Actions}Q^{*,\epsilon}(\state',\action')]-E_{\state'\sim\transitionprob(\cdot|\state,\action)}[\max_{\action''}Q^{*}(\state',\action'')]  \\
  \leq & 
...
%\footnote{\YM{should we move to the Harm van Seijen, Richard S. Sutton: True Online TD(lambda). ICML 2014: 692-700}}
...
Note that we removed the discount factor, and also explicitly used the fact that the value of a goal state is $0$. The latter is critical for the algorithm to converge, under the Assumption \ref{ass:finite_termination_prob} that a goal state will always be reached.
...
%The $Q$-learning outline of the asymptotic convergence and 
...
% and developed by Blum \cite{Blum54}. 
