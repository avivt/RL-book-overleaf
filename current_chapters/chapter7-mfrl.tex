...
\begin{algorithmic}[1]
\State \textbf{Initialize:} Set $\widehat{\QValue}(\state,\action) = 0$, for all $\state$, $\action$.
...
\State \textbf{For} {$\ttime = 0, 1, 2, \dots$}
    ...
    \State \quad Update:
    \[
    \widehat{V}(\state_\ttime) = \widehat{V}(\state_\ttime) + \alpha_\ttime(\state_\ttime, \action_\ttime) \left[\reward_\ttime + \discount \widehat{V}(\state_{\ttime+1}) - \widehat{V}(\state_\ttime)\right]
    \]
    \quad where $\alpha_\ttime(\state, \action)$ is the step size for $(\state, \action)$ at time $\ttime$.
\end{algorithmic}
\end{algorithm}
...
\State \textbf{For} {$\ttime = 0, 1, 2, \dots$}
    ...
    \State \quad Update:
    \[
    Q_{\ttime+1}(\state_\ttime, \action_\ttime) := Q_\ttime(\state_\ttime, \action_\ttime) + \alpha_\ttime(\state_\ttime, \action_\ttime) \left[\reward_\ttime + \gamma \max_{\action'} Q_\ttime(\state_{\ttime}', \action') - Q_\ttime(\state_\ttime, \action_\ttime)\right]
    \]
\end{algorithmic}
\end{algorithm}
...
\[
Q_\ttime =
(1-\frac{1}{\ttime^\theta})Q_{\ttime-1}+\frac{1}{\ttime^\theta}[0+\discount
Q_{\ttime-1}]=(1-\frac{1-\discount}{\ttime^\theta})Q_{\ttime-1}
\]
When we solve the recursion we get that
...
\subsection{Algorithms for Episodic MDPs}\label{ssec:mfrl_episodic}
