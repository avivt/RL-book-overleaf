...
% \item 
Recall from Chapter \ref{chapter:learning-model-free} that without function approximation, TD methods effectively compute a solution to the Bellman equation. With function approximation, this is no longer the case, as it is clear that whatever solution TD methods converge to (if they converge at all), must take the function approximation into account. We shall start our investigation from a fundamental equation that takes function approximation into account -- the projected Bellman equation (PBE). We will use the PBE to define a particular approximation of the value function, and study its properties. We will later develop TD algorithms that estimate this approximation using sampling.
...
\State \textbf{For} {$\ttime = 0, 1, 2, \dots$}
    ...
    \State \quad Update:
    \begin{equation*}
    \weights_{\ttime+1} = \weights_\ttime + \alpha_\ttime \left( \reward(\state_\ttime,\action_\ttime) + \discount f(\state_{\ttime+1},\action_{\ttime+1};\weights_\ttime) - f(\state_{\ttime},\action_{\ttime}; \weights_\ttime) \right) \nabla_{\weights} f(\state_\ttime,\action_\ttime,\weights)
\end{equation*}
\end{algorithmic}
\end{algorithm}
...
It is also possible to collect data from the modified policy at each iteration $k$, instead of the initial policy.
