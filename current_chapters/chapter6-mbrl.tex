...
\begin{proof}
The difference in the rewards can result in a maximum return difference of $\alpha T$. The for difference in the dynamics,
...
\begin{proof}
The difference in the rewards can result in a maximum return difference of $\alpha /(1-\discount)$. The for difference in the dynamics,
...
\lambda\right]\leq 2^{k+1} e^{-n\lambda^2/2}
\]
\end{lemma}
...
\]
This holds for $m=O\left(\frac{V^2_{max}}{\varepsilon'^2}\log (N
|\States|\;|\Actions|/\delta)\right)$, where $V_{max}$ bounds the maximum
...
Therefore, if we sample $m=O\left(\frac{V^2_{max}}{(1-\discount)^2\varepsilon'^2}\log \frac{N
|\States| \;|\Actions|}{\delta}\right)$, we have that with probability at least
$1-\delta$ for every state-action $(\state,\action)$ the approximation error is
...
state $\state_1$ (this state is not a ``real state'' but is useful for defining the algorithm and in the analysis). For each \texttt{known} state-action $(\state,\action)$, we set the next
...
% \end{proof}
\begin{theorem}
With probability at least $1-\delta$, algorithm \texttt{R-MAX} will  be $\varepsilon$-optimal, i.e., have an expected return at least  $V^*-\varepsilon$, in all iterations except for at most 
\[
m|\States|\;|\Actions|\frac{2\tHorizon\Rmax}{\varepsilon}
\]
iterations.
