...
%\footnote{This is a worse case guarantee. A more refined guarantee can take in to account the probability of reaching state $\state$} 
...
Since $\widehat{V}_0(\state) = \Value_0(\state)$, the recurrence above gives:
\[
...
% $\tilde{\reward}(\state,\action)=\Rmax$ for every $(\state,\action)$. Initialize $\ttime=0$, and $\state_0\in \States$ arbitrarily. At
...
% \State \quad (Note: this only has an effect the first time we visit $(\state_\ttime, \action_\ttime)$.)
\end{algorithmic}
\end{algorithm}
...
% Initially set $\widehat{M}_0$ to have $\widehat{f}(\state,\action)=\state$ and
% $\tilde{\reward}(\state,\action)=\Rmax$ for every $(\state,\action)$. Initialize $\ttime=0$, and $\state_0 \in \States$ arbitrarily. At
% time $\ttime$ do the following.
% \begin{enumerate}
% %\item
...
% \State \quad \quad (Note: this has an effect only the first time we observe $(\state_\ttime, \action_\ttime)$)
\end{algorithmic}
\end{algorithm}
...
% The algorithm has three parameters: (1) $m$, how many
...
% following, in the explore stage of the algorithm.
% \begin{enumerate}
% \item
% Compute $\tilde{\policy}^*_\ttime$, the optimal policy for
% $\widetilde{M}$, for the finite horizon return with horizon $\tHorizon$ and initial state $\state_0$.
% \item
% If the expected return of $\tilde{\policy}^*_\ttime$ on $\widetilde{M}$
% is less than $\varepsilon/2$, then terminate.
% \item
% Run policy $\tilde{\policy}^*_\ttime(\state_{\ttime})$ starting at state $\state_0$ and observe a trajectory $(\state_0,\action_0,\reward_0,\state_1, \ldots, \state_\tHorizon)$.
% %Use $\action_\ttime=\tilde{\policy}^*_\ttime(\state_{\ttime})$.
% \item
% %Observe the reward $\reward_\ttime$ and the next state
% %$\state_{\ttime+1}$ and add
...
% update the relevant $\widetilde{M}$ entries for $(\state,\action)$.
% \end{enumerate}

% At termination we start the exploit phase of the algorithm.
% We define an MDP $M'$ as follows. For each \texttt{known} state-action pair
...
% \AT{clarify what is the initial state in each rollout. Also I think we can remove dependence on $\tHorizon$ for stationary model.}

% \YM{Added an initial state to the algorithm (maybe we should move the algorithm to an algorithm environment? also in the MAB? }

\YM{Aviv: not sure what you mean be removing the dependency on $\tHorizon$, we can adjust $\alpha$ and $m$ and have the difference be only $\epsilon$, this is what you want?}
\AT{If I understand correctly, in the part "By Lemma~\ref{lemma:Bretagnolle Huber-Carol}, it is sufficient to have that $m\geq
c\frac{|\States|+\log(\tHorizon|\States|\;|\Actions|/\delta)}{\alpha^2}$", the $\tHorizon$ is due to assuming a non-stationary model (a union bound over each state, action, and time). So if the model is stationary we can remove $\tHorizon$. Right?}
...
known}. 
% Otherwise, the {\tt R-MAX} algorithm would be very similar
...
% {\em Execution:} At iteration $\ttime$. (1) Build a model
...
\vspace{1ex}
\State \textbf{For} $\ttime = 0, 1, 2, \dots$
% \State \quad Build model $\widehat{M}_\ttime$ (see "Model Construction" below)
...
% \end{lemma}
% \begin{proof}
% Let $y_1$ denote the number of trials until the first success, $y_2$ denote the number of additional trials until the second success, an so on. We have that the $y_i$'s are i.i.d. Geometric random variables, and that $k = \sum_{i=1}^n y_i$.
...
% \AT{I'm not sure how this result follows from the claim "the expected
% number of such iterations is at most
% $m|\States|\;|\Actions|\frac{2\tHorizon\Rmax}{\varepsilon}$". How do we go from expectation to a strict bound?}

% \YM{Each time is a Bernoulli random variable, so we can get a high probability, but you are right that we need to add a $log(1/\delta)$ factor and probably adjust the constant (Done!)}

\YM{Do we want to make the algorithm here in an algorithm environment? Seems now it looks better!}
...
Another line of model-based learning algorithms is based on learning the dynamics, without considering the rewards. Later, the learner can adapt to any reward function and derive an optimal policy for it, which is also named ``Best Policy Identification (BPI)".
...
%The first polynomial time model based learning algorithm is $E^3$ (Explicit Explore or Exploit) of \cite{KearnsS02}. 
%While we did not outline the $E^3$ algorithm, we did describe the effective horizon and the simulation lemma from there.

%The improved sampling bounds for the optimal policy using approximate Value Iteration is following \cite{KearnsS98a}.



%Analysis of related models, especially the {\tt PAC-MDP} model appears in \cite{StrehlLL09,Li2012}.
