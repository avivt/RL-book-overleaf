...
Fix a policy $\policy\in SD$, stationary and deterministic. The goal
...
\paragraph{Maximum Likelihood model} Recall that the empirical model, which is also the
...
\[
V_{\ttime+1}(\state_\ttime)=(1-\alpha_\ttime)V_\ttime+\alpha_\ttime
\Phi_\ttime
\]
...
\paragraph{Comparing various algorithms:}
...
averaging parameter $\lambda$ is used to average over the different
...
$\tau<t$ where $\state_\tau=\state=\state_\ttime$. The influence is
exactly $(\discount\lambda)^{t-\tau}\Delta_\ttime$. This implies
that for every such $\tau$ we can do the desired update, however, we
can aggregate all those updates to a single update. Let,
\[
e_\ttime(\state)=\sum_{\tau\leq t: \state_\tau=\state}
(\discount\lambda)^{t-\tau} = \sum_{\tau=1}^\ttime
(\discount\lambda)^{t-\tau}I(\state_\tau=\state)
\]
...
$e_\ttime(\state)=\sum_{k=0}^\ttime(\lambda\discount)^{t-k}I(\state=\state_k)$.
...
$t+i$ and sum over $n$, $k$ and $\ttime$, in the forth identity we
...
\begin{align}
\sum_{\ttime=0}^{\infty } \Delta \Value^B_\ttime (\state)&=
\sum_{\ttime=0}^{\infty} \alpha
